...
function TooltipBox({ active, payload }: any) {
  ...
  const d = payload[0]?.payload as RankedCandidate;
  if (!d) return null;
  return (
    <div className="rounded-lg border border-border bg-popover p-2.5 text-[11px] shadow-lg max-w-[200px]">
      ...
      <div className="mt-1 space-y-0.5">
        <div>Algo rank: <span className="font-mono font-bold text-foreground">#{d.algo_rank}</span></div>
        <div>Final rank: <span className="font-mono font-bold text-foreground">#{d.rank}</span></div>
        <div>CE score: <span className="font-mono font-bold text-ai">{Math.round(d.ce_score)}</span></div>
      </div>
    </div>
  );
}
...
export default function CEInspectorPage() {
  ...
  const ceHist = useMemo(() => {
    ...
    results.forEach(c => {
      const i = Math.min(9, Math.floor(c.ce_score / 10));
      buckets[i].count++;
    });
    const cutScore = results[results.length - 1]?.ce_score ?? 40;
    return buckets.map((b, i) => ({ ...b, aboveCut: i * 10 >= cutScore }));
  }, [results]);

  const promotions = useMemo(() =>
    results
      .map(c => ({ ...c, delta: c.algo_rank - c.rank }))
  ...
  const avgCE = (results.reduce((sum, c) => sum + c.ce_score, 0) / results.length).toFixed(1);
  const algoCECorr = 0.72; // Pearson correlation (simulated)
  const topPromoted = results.filter(c => (c.algo_rank - c.rank) >= 10).length;
  const topDemoted = results.filter(c => (c.rank - c.algo_rank) >= 10).length;
              ...
              <ScatterChart>
                ...
                <Scatter
                  ...
                  shape={(props: any) => {
                    ...
                    const delta = payload.algo_rank - payload.rank;
                    const color = payload.ce_score >= 80 ? "hsl(142 72% 29%)" : payload.ce_score >= 60 ? "hsl(var(--ai))" : payload.ce_score >= 40 ? "hsl(38 92% 50%)" : "hsl(var(--muted-foreground))";
              ...
              {[...promotions, ...demotions].map((c, i) => {
                ...
                return (
                  <tr key={c.candidate_id + i} className={`border-b border-border/40 hover:bg-muted/30 transition-colors ${i === 4 ? "border-b-2 border-border" : ""}`}>
                    ...
                    <td className="px-4 py-2 font-mono text-foreground">#{c.algo_rank}</td>
                ...
                {loggedResults.map(c => {
                  ...
                  return (
                    <tr key={c.candidate_id} className="border-b border-border/30 hover:bg-muted/20 transition-colors">
                      ...
                      <td className="px-3 py-1.5 font-mono text-foreground">{c.skill_score.toFixed(3)}</td>
                      <td className="px-3 py-1.5">
                        <span className="ai-badge text-[10px] font-mono px-1 py-0.5 rounded-[3px]">{Math.round(c.ce_score)}</span>
                      </td>
                      <td className="px-3 py-1.5 font-mono text-foreground">{c.final_score.toFixed(3)}</td>
                      <td className="px-3 py-1.5">
